...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!stamp_id || !admin_id) {
      return json({ error: "Missing required fields: stamp_id, admin_id" }, 400)
    }

    const { data: stamp, error: stampError } = await supabase
      .from("crawl_stamps")
      .select("id, user_id, crawl_id, is_verified, verification_note")
      .eq("id", stamp_id)
      .single()
    ...
    if (stamp.is_verified) {
      return json({ error: "Stamp is already verified" }, 409)
    }

    const note = verification_note ?? stamp.verification_note

    const { error: updateError } = await supabase
      .from("crawl_stamps")
      .update({ is_verified: true, verification_note: note })
      .eq("id", stamp_id)

    if (updateError) {
      return json({ error: "Failed to restore stamp" }, 500)
    }
    ...
    console.error("admin-restore-stamp error:", err)
